...
Guardrails:
- **Drift guard**: seeding is label string match. A topic whose words aren't node labels drifts silently — ranking can put unrelated files above the real target. Retry once with graphify's installed vocab-expansion guidance before concluding greenfield.
...
- Graph absent or stale → fall back to the legacy grep/investigator path (see `phase1/PROCEDURE.md` Step 1).
